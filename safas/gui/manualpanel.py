...
from safas.comp.loader_util import loadUi
...
class ManualPanel(QMainWindow):
    ...
    def __init__(self, parent=None, params=None, params_file=None, *args, **kwargs):
        super(ManualPanel, self).__init__(*args, **kwargs)
        loadUi("safas/gui/manualtracking.ui", self)

        self.parent = parent

        # connect buttons
        for key in ["filter", "start", "end", "remove","clear_all","save","merge"]: 
            getattr(self, f"button_{key}").clicked.connect(getattr(self, f"click_{key}"))
        ...
        # predictone.activated.connect(self.click_next)
    ...
    def click_start(self): 
        """ """
        self.radio_processing.setChecked(True)
...
#         self.parent = parent
#         self.setWindowTitle('Manual Tracking')
